...
if (isset($_POST['request'])) {
	...
	if (!$post_provider || !$post_pembayaran || !$post_jumlah) {
	...
	} else if ($post_jumlah < 15000) {
		$_SESSION['hasil'] = array('alert' => 'danger', 'judul' => 'Permintaan Gagal', 'pesan' => 'Minimal Deposit Pulsa 15000.');
		...
		$insert = $conn->query("INSERT INTO deposit_tsel VALUES ('','$kode', '$sess_username', '".$data_metod['tipe']."', '".$data_metod['provider']."' ,'$metodnya', '$post_pengirim','".$data_metod['tujuan']."','$post_jumlah', '$get_saldo', 'Pending', 'Website', '$date', '$time')");
		if ($insert == TRUE) {
			exit(header("Location: ".$config['web']['url']."invoice-pulsa"));
...
<div class="row">
	<div class="col-md-7">
		<div class="card">
			<div class="card-body">
				<h4 class="text-uppercase text-center header-title"><i class="ti-wallet text-primary"></i> DEPOSIT PULSA</h4>
                <hr>
				<form class="form-horizontal" role="form" method="POST">
					...
					<div class="form-group">
						...
						<div class="col-md-12">
							<select class="form-control" name="provider" id="provider">
								...
								<?php
								$cek_kategori = $conn->query("SELECT * FROM metode_depo WHERE tujuan NOT IN ('DPEDIA') AND tipe = 'Pulsa' AND keterangan = 'ON' ORDER BY nama ASC");
					...
					<!--
					<div class="form-group">
						<label class="col-md-12 control-label">Saldo yang didapat</label>
						<div class="col-md-12">
							<div class="input-group">
								<div class="input-group-prepend">
									<span class="input-group-text">Rp</span>
					...
					<div class="form-group">
						<label class="col-md-12 control-label">Nomor Pengirim *<br/>
							<small class="text-danger">Masukkan Awalan 62. Bukan 08 / +62</small>
						</label>
						<div class="col-md-12">
							<input type="text" class="form-control" name="pengirim" placeholder="6282377XXXXXX" id="pengirim">
						</div>
					</div>
		...
		<div class="card">
			<div class="card-body">
				...
				<div class="table-responsive">
					...
					<ol class="list-p">
						<li>Minimal deposit Rp.15.000.</li>
